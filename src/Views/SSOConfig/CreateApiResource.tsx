...
const CreateApiResource = (props: IProps) => {
    ...
    useEffect(() => {
        if (props.apiResourceCreate.Visible)
            resetForm();
    }, [props.apiResourceCreate.Visible])
    ...
    const formSubmitHandler = () => {
        if (onFormSubmit()) {
            props.saveApiResource(values);
        } else
            props.pushAlert({ title: t('userError'), description: t('DataIsIncomplete'), variant: 'warning' })
    }
    const Buttons = () => {
        return <React.Fragment>
            <button type="button" className="btn btn-outline-secondary" onClick={() => props.toggleCreateApiResourceModal(false)}>{t("cancel")}</button>
            <button className="btn btn-success px-4 ml-1" onClick={formSubmitHandler}>{t("save")}</button>
        </React.Fragment>
    }
    return (
        <GymModal ModalTitle={t("createApiResource")} Visible={props.apiResourceCreate.Visible} onCancel={() => props.toggleCreateApiResourceModal(false)} buttons={<Buttons />}>
