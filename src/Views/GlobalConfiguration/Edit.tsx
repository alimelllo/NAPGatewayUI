...
const GlobalConfigurationUpdate = (props: IProps) => {
    ...
    { value: "ftp", label: "ftp" }];
    useEffect(() => {
        setInitialValues(props.globalConfigurationUpdate.item)
    }, [props.globalConfigurationUpdate.item])
    const formSubmitHandler = () => {
        if (onFormSubmit()) {
            props.updateGlobalConfiguration(values);
        } else
            props.pushAlert({ title: t('userError'), description: t('DataIsIncomplete'), variant: 'warning' })
    }
    const Buttons = () => {
        return <React.Fragment>
            <button type="button" className="btn btn-outline-secondary" onClick={() => props.toggleUpdateGlobalConfigurationModal({}, false)}>{t("cancel")}</button>
            <button className="btn btn-success px-4 ml-1" onClick={formSubmitHandler}>{t("save")}</button>
        </React.Fragment>
    }
    return (
        <GymModal ModalTitle={t("update")} size="modal-xl" Visible={props.globalConfigurationUpdate.Visible} onCancel={() => props.toggleUpdateGlobalConfigurationModal({}, false)} buttons={<Buttons />}>
            <div className="row">
