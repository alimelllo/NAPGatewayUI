...
const AddUpstreamHeaderTransformModal = (props: IProps) => {
    ...
    useEffect(() => {
        if (props.upstreamHeaderTransformAdd.Visible)
            resetForm([{ fileConfigurationId: props.upstreamHeaderTransformAdd.fileConfigurationId }, { fileRouteId: props.upstreamHeaderTransformAdd.fileRouteId }]);
    }, [props.upstreamHeaderTransformAdd.Visible])
    const formSubmitHandler = () => {
        if (onFormSubmit()) {
            props.addUpstreamHeaderTransform(values,props.routeId);
        } else
            props.pushAlert({ title: t('userError'), description: t('DataIsIncomplete'), variant: 'warning' })
    }
    const Buttons = () => {
        return <React.Fragment>
            <button type="button" className="btn btn-outline-secondary" onClick={() => props.toggleAddUpstreamHeaderTransformModal(0, 0, false)}>{t("cancel")}</button>
            <button className="btn btn-success px-4 ml-1" onClick={formSubmitHandler}>{t("save")}</button>
        </React.Fragment>
    }
    return (
        <GymModal ModalTitle={t("addUpstreamHeaderTransform")} Visible={props.upstreamHeaderTransformAdd.Visible} onCancel={() => props.toggleAddUpstreamHeaderTransformModal(0, 0, false)} buttons={<Buttons />}>
...
export default connect(
    ...
    routeActions,
)(AddUpstreamHeaderTransformModal);