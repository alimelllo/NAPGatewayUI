...
const AddPreAuthenticationPartyBodyModal = (props: IProps) => {
  ...
  useEffect(() => {
    if (props.preAuthenticationPartyBodyAdd.Visible)
      resetForm([
        {
          fileConfigurationId:
            props.preAuthenticationPartyBodyAdd.fileConfigurationId,
        },
        { fileRouteId: props.preAuthenticationPartyBodyAdd.fileRouteId },
      ]);
  }, [props.preAuthenticationPartyBodyAdd.Visible]);
  const formSubmitHandler = () => {
    if (onFormSubmit()) {
      props.addPreAuthenticationPartyBody(values, props.routeId);
    } else
      props.pushAlert({
        title: t("userError"),
    ...
    return (
      <React.Fragment>
        <button
          ...
          onClick={() =>
            props.toggleAddPreAuthenticationPartyBodyModal(0, 0, false)
          }
        >
          {t("cancel")}
        </button>
  ...
  return (
    <GymModal
      ModalTitle={t("addPreAuthenticationPartyBody")}
      Visible={props.preAuthenticationPartyBodyAdd.Visible}
      onCancel={() =>
        props.toggleAddPreAuthenticationPartyBodyModal(0, 0, false)
      }
      buttons={<Buttons />}
...
export default connect(
  ...
  routeActions
)(AddPreAuthenticationPartyBodyModal);
