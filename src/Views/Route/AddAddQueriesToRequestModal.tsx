...
const AddAddQueriesToRequestModal = (props: IProps) => {
  ...
  useEffect(() => {
    if (props.addQueriesToRequestAdd.Visible)
      resetForm([
        {
          fileConfigurationId: props.addQueriesToRequestAdd.fileConfigurationId,
        },
        { fileRouteId: props.addQueriesToRequestAdd.fileRouteId },
      ]);
  }, [props.addQueriesToRequestAdd.Visible]);
  const formSubmitHandler = () => {
    if (onFormSubmit()) {
      props.addAddQueriesToRequest(values, props.routeId);
    } else
      props.pushAlert({
        title: t("userError"),
    ...
    return (
      <React.Fragment>
        <button
          ...
          onClick={() => props.toggleAddAddQueriesToRequestModal(0, 0, false)}
        >
          {t("cancel")}
        </button>
        <button
  ...
  return (
    <GymModal
      ModalTitle={t("addAddQueriesToRequest")}
      Visible={props.addQueriesToRequestAdd.Visible}
      onCancel={() => props.toggleAddAddQueriesToRequestModal(0, 0, false)}
      buttons={<Buttons />}
    >
