...
const AddRouteClaimsRequirementModal = (props: IProps) => {
    ...
    useEffect(() => {
        if (props.routeClaimsRequirementAdd.Visible)
            resetForm([{ fileConfigurationId: props.routeClaimsRequirementAdd.fileConfigurationId }, { fileRouteId: props.routeClaimsRequirementAdd.fileRouteId }]);
    }, [props.routeClaimsRequirementAdd.Visible])
    const formSubmitHandler = () => {
        if (onFormSubmit()) {
            props.addRouteClaimsRequirement(values,props.routeId);
        } else
            props.pushAlert({ title: t('userError'), description: t('DataIsIncomplete'), variant: 'warning' })
    }
    const Buttons = () => {
        return <React.Fragment>
            <button type="button" className="btn btn-outline-secondary" onClick={() => props.toggleAddRouteClaimsRequirementModal(0, 0, false)}>{t("cancel")}</button>
            <button className="btn btn-success px-4 ml-1" onClick={formSubmitHandler}>{t("save")}</button>
        </React.Fragment>
    }
    return (
        <GymModal ModalTitle={t("addRouteClaimsRequirement")} Visible={props.routeClaimsRequirementAdd.Visible} onCancel={() => props.toggleAddRouteClaimsRequirementModal(0, 0, false)} buttons={<Buttons />}>
...
export default connect(
    ...
    routeActions,
)(AddRouteClaimsRequirementModal);