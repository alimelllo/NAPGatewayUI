...
const LoadBalancerOptionsUpdate = (props: IProps) => {
  ...
  const formSubmitHandler = () => {
    if (onFormSubmit()) {
      props.setLoadBalancerOptions(values, props.routeId);
    } else
      props.pushAlert({
        title: t("userError"),
    ...
    return (
      <React.Fragment>
        <button
          ...
          onClick={() => props.toggleSetLoadBalancerOptionsModal({}, false)}
        >
          {t("cancel")}
        </button>
        <button
  ...
  return (
    <GymModal
      ModalTitle={t("updateLoadBalancerOptions")}
      Visible={props.loadBalancerOptionsSet.Visible}
      onCancel={() => props.toggleSetLoadBalancerOptionsModal({}, false)}
      buttons={<Buttons />}
    >
