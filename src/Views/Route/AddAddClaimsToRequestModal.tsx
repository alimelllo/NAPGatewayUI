...
const AddAddClaimsToRequestModal = (props: IProps) => {
  ...
  useEffect(() => {
    if (props.addClaimsToRequestAdd.Visible)
      resetForm([
        {
          fileConfigurationId: props.addClaimsToRequestAdd.fileConfigurationId,
        },
        { fileRouteId: props.addClaimsToRequestAdd.fileRouteId },
      ]);
  }, [props.addClaimsToRequestAdd.Visible]);
  const formSubmitHandler = () => {
    if (onFormSubmit()) {
      props.addAddClaimsToRequest(values, props.routeId);
    } else
      props.pushAlert({
        title: t("userError"),
    ...
    return (
      <React.Fragment>
        <button
          ...
          onClick={() => props.toggleAddAddClaimsToRequestModal(0, 0, false)}
        >
          {t("cancel")}
        </button>
        <button
  ...
  return (
    <GymModal
      ModalTitle={t("addAddClaimsToRequest")}
      Visible={props.addClaimsToRequestAdd.Visible}
      onCancel={() => props.toggleAddAddClaimsToRequestModal(0, 0, false)}
      buttons={<Buttons />}
    >
