...
const AddDownstreamHeaderTransformModal = (props: IProps) => {
  ...
  useEffect(() => {
    if (props.downstreamHeaderTransformAdd.Visible)
      resetForm([
        {
          fileConfigurationId:
            props.downstreamHeaderTransformAdd.fileConfigurationId,
        },
        { fileRouteId: props.downstreamHeaderTransformAdd.fileRouteId },
      ]);
  }, [props.downstreamHeaderTransformAdd.Visible]);
  const formSubmitHandler = () => {
    if (onFormSubmit()) {
      props.addDownstreamHeaderTransform(values, props.routeId);
    } else
      props.pushAlert({
        title: t("userError"),
    ...
    return (
      <React.Fragment>
        <button
          ...
          onClick={() =>
            props.toggleAddDownstreamHeaderTransformModal(0, 0, false)
          }
        >
          {t("cancel")}
        </button>
  ...
  return (
    <GymModal
      ModalTitle={t("addDownstreamHeaderTransform")}
      Visible={props.downstreamHeaderTransformAdd.Visible}
      onCancel={() =>
        props.toggleAddDownstreamHeaderTransformModal(0, 0, false)
      }
      buttons={<Buttons />}
...
export default connect(
  ...
  routeActions
)(AddDownstreamHeaderTransformModal);
