...
const CalendarPage: React.FC<IProps> = (props: IProps) => {
    ...
    const [mainDate, setDate] = useState<NDate>(new NDate(props.theDate))
    const [mainYear, setYear] = useState<number>(new NDate().yearJalali);
    ...
    useEffect(() => {
        let selectDate = new NDate(props.theDate);
        let year = selectDate.yearJalali;
        const theMonthIndex = selectDate.monthJalali;
        const selectDay = selectDate.dayJalali;
        setDate(selectDate)
        setYear(year)
        setMonthIndex(theMonthIndex)
        setMonth(theMonthIndex)
        setday(selectDay)
    ...
    const createMonth = (year: number, monthIndex: number) => {
        const cal = new NDate([year,monthIndex,1])
        let currentSelectYear = year;
        let theDaysOfMonth = daysOfMonth[monthIndex - 1];
        ...
        for (let i = 0; i < totalDays; i++) {
            if (i < dayOfWeek) {
                ...
                daysArray.push(new NDate([currentSelectYear, monthIndex, day]));
                day++;
            } else {
                daysArray.push(null)
            }
    ...
    const pervMonth = (currentMonth: number) => {
        ...
        const newDate = new NDate([theMainYear, currentMonth, 1]);
        setDays(createMonth(theMainYear, currentMonth));
        setYear(theMainYear)
        setDate(newDate)
    ...
    const daySelected = (y: number, m: number, d: number) => {
        const newDate = new NDate(`${y}-${m}-${d} ${mainHour}:${mainMinute}`);
        if (!props.setTime) {
    ...
    const theMonthSelected = (theMonth: number) => {
        ...
        setMonth(theMonth);
        setDays(createMonth(mainYear, theMonth))
        resetModals();
        showDaysModal(true);
        changeView();
    };
    ...
    const theMinuteSelected = (m: number) => {
        setMinute(m);
        setDays(createMonth(mainYear, mainMonth))
        resetModals()
        showDaysModal(true)
        changeView();
        props.sendDate({
